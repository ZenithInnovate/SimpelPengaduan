...
- **Standarisasi Respon Pesan & File Translasi messages.php (2026-09-16)**: Menghapus berkas duplikat `pesan.php` dan menstandarisasi langsung ke `Lang/id/messages.php` dengan penamaan kunci Bahasa Inggris (`response_success`, `update_status_success`, `delete_success`) dengan nilai dan DocBlock Bahasa Indonesia serta alias kompatibilitas mundur. Menyelaraskan respon CRUD JSON `PengaduanController` menggunakan helper `simpel_message()` dan translasi `simpel-pengaduan::messages.*` (CODING-STANDARDS.md §33).
