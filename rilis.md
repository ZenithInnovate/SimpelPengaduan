##### 🔒 **CATATAN RILIS v1.0.1**
- **Standarisasi Kunci Translasi Bahasa Inggris pada Lang/id/pesan.php (2026-09-16)**: Menstandarisasi penamaan kunci pesan translasi menggunakan Bahasa Inggris (`response_success`, `update_status_success`, `delete_success`) dengan nilai dan DocBlock Bahasa Indonesia serta alias kompatibilitas mundur (CODING-STANDARDS.md §33).
- **Konsistensi Penggunaan Carbon:: pada Service dan Layout (2026-09-16)**: Menyelaraskan seluruh operasi tanggal dan waktu pada `Services/PengaduanService.php` dan tampilan footer `Views/frontend/layouts/app.blade.php` menggunakan `Carbon::` terpusat sesuai CODING-STANDARDS.md §32 & §34.
- **Standarisasi Respon Pesan via File Translasi Lang (2026-09-16)**: Menambahkan kamus pesan modul `Lang/id/pesan.php` dan menyelaraskan respon CRUD JSON PengaduanController menggunakan helper `simpel_pesan()` dan translasi `simpel-pengaduan::pesan.*` (CODING-STANDARDS.md §33).
- **Standarisasi Penggunaan simpel_url(), simpel_setting(), dan simpel_identitas() (2026-09-15)**: Mengonversi seluruh resolusi URL aplikasi internal ke helper adaptif `simpel_url()`, pembacaan setting ke `simpel_setting()`, dan pembacaan identitas desa ke `simpel_identitas()` pada controller, seeder, dan seluruh view blade per CODING-STANDARDS.md §23, §24, & §26.
- **Penguatan Keamanan Upload Foto Pengaduan (2026-09-15)**: `PengaduanService::uploadFoto()` kini menggunakan `folder($targetDir, '0755', 'htaccess1')` (bukan `File::makeDirectory` polos) agar direktori unggahan pengaduan otomatis terlindungi proteksi anti-eksekusi PHP. Ekstensi nama file kini diturunkan dari `getMimeType()` (MIME type sesungguhnya via `finfo`) bukan `getClientOriginalExtension()` yang bisa dipalsukan penyerang (Issue 017).
